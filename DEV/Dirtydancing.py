...
# 1) Locate this script’s folder and the DEV subfolder
script_dir = Path(__file__).parent
dev_dir    = script_dir / "DEV"
# DEBUG: list every file in DATA folder
print("Files in", script_dir)
for p in script_dir.iterdir():
    print("  •", p.name)

# Point at the JSON file next to Dirtydancing.py
file_path = script_dir / "cdw_sapp_customer.json"

# Now load:
customer_json = dev_dir / "cdw_sapp_customer.json"
with open(customer_json, "r") as f:
    customers = json.load(f)



# Helper functions based on mapping logic
...
def format_phone(number):
    num_str = str(number).zfill(10)
    return f"({num_str[:3]}){num_str[3:6]}-{num_str[6:]}"

def full_street_address(apt_no, street_name):
    return f"{apt_no}, {street_name}"
...
for customer in customers:
    transformed = {
        "SSN": customer["SSN"],
        "FIRST_NAME": to_title_case(customer["FIRST_NAME"]),
        "MIDDLE_NAME": to_lower(customer["MIDDLE_NAME"]),
        "LAST_NAME": to_title_case(customer["LAST_NAME"]),
        "Credit_card_no": customer["CREDIT_CARD_NO"],
        "FULL_STREET_ADDRESS": full_street_address(customer["APT_NO"], customer["STREET_NAME"]),
        "CUST_CITY": customer["CUST_CITY"],
        "CUST_STATE": customer["CUST_STATE"],
        "CUST_COUNTRY": customer["CUST_COUNTRY"],
        "CUST_ZIP": customer["CUST_ZIP"],
        "CUST_PHONE": format_phone(customer["CUST_PHONE"]),
...
customer_out = script_dir / "cdw_sapp_customer_transformed.json"
with open(customer_out, "w") as f:
    json.dump(transformed_customers, f, indent=4)
...
# 4) BRANCH JSON
branch_json = dev_dir / "cdw_sapp_branch.json"
with open(branch_json, "r") as f:
    branch_data = json.load(f)
branch_df = pd.json_normalize(branch_data)
branch_df["LAST_UPDATED"] = pd.to_datetime(branch_df["LAST_UPDATED"])
branch_csv_path = script_dir / "cleaned_cdw_sapp_branch.csv"
branch_df.to_csv(branch_csv_path, index=False)


# 5) CREDIT JSON
# ─── 3) CREDIT JSON ─────────────────────────────────────────────────────────
credit_json = dev_dir / "cdw_sapp_credit.json"
with open(credit_json, "r") as f:
    credit_data = json.load(f)

credit_df = pd.json_normalize(credit_data)

# Create TIMEID
credit_df["TIMEID"] = (
    credit_df["YEAR"].astype(str).str.zfill(4)
  + credit_df["MONTH"].astype(str).str.zfill(2)
  + credit_df["DAY"].astype(str).str.zfill(2)
)

# Rename fields per your mapping
...
# Select and order only the target columns
ordered_columns = [
    "TRANSACTION_ID",
    "CUST_CC_NO",
    "TIMEID",
...
credit_cleaned_df = credit_df[ordered_columns]

# Save to CSV
credit_csv_path = script_dir / "cleaned_cdw_sapp_credit.csv"
credit_cleaned_df.to_csv(credit_csv_path, index=False)
...
df_customer = pd.DataFrame(transformed_customers)
